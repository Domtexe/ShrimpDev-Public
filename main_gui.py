...
def _r2296_log_import_paths_once() -> None:
    try:
        ...
        line = (
            f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] [BOOT] import_paths "
            + " | ".join(parts)
        )
...
class ShrimpDevApp(tk.Tk):
    def __init__(self) -> None:
        ...
        try:
            ...
            if hasattr(module_agent, "build_agent_tab"):
                ...
                _tk.Label(self.tab_agent, text="Agent: build_agent_tab fehlt.").pack(
            ...
            try:
                ...
                _tk.Label(self.tab_agent, text="Agent-Tab Fehler: " + repr(_exc)).pack(
                    anchor="w", padx=8, pady=8
                )
            except Exception:
                pass
    ...
    def _build_intake(self, parent: tk.Widget) -> None:
        ...
        def _on_led_syntax():
            ...
            try:
                widget = (
                    getattr(self, "txt_intake", None)
                    or getattr(self, "txt_code", None)
                    or getattr(self, "intake_text", None)
                )
...
def main() -> None:
    ...
    app = ShrimpDevApp()
    # R2795: bind tkinter to logic_actions popup subsystem (required for r1851 popups)
    try:
        from modules import logic_actions
        import tkinter as _tk
        logic_actions._r1851_bind_tk(_tk)
    except Exception as _exc:
        print(f"[R2795] WARN: failed to bind tkinter to logic_actions: {_exc}")
