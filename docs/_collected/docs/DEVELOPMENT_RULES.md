...
<!-- MR-RUNNER-DEFECT-NO-FIX-RUNNER -->

## Masterregel: Defekter Runner wird neu ausgeliefert (kein Fix-Runner)

Wenn ein Runner (`tools/R####.cmd/.py`) defekt ist (z. B. SyntaxError, Crash beim Start, falscher Entry-Point):

1. **Keinen Reparatur-Runner bauen**, der den Runner repariert.
2. Stattdessen den **betroffenen Runner korrigiert vollständig neu ausgeben** (Datei 1:1 ersetzen).
3. Optional: **Neue Runner-Nummer** nur dann, wenn der ursprüngliche Runner bereits produktiv genutzt wurde und ein Audit-Trail nötig ist.
4. Immer: **Minimaltest** angeben/ausführen:
   - `python -m py_compile tools/R####.py`

Ziel: Keine Meta-Kaskaden („Runner repariert Runner repariert Runner“), weniger Chaos, schneller wieder stabil.
