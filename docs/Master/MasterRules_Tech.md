# MasterRules_Tech – Technische Architektur

_Automatisch generiert oder aktualisiert durch R2037 am 2025-12-09 09:39:55_

## 1. Trennung ShrimpDev / ShrimpHub

- ShrimpDev ist Entwicklungsumgebung und Patch-System für ShrimpHub.
- ShrimpHub ist Produktions-GUI für Medien- und Automationsworkflows.

## 2. Modulschnittstellen

- Kernlogik liegt in `modules/`, Runner unter `tools/`, GUI in `main_gui.py`
  plus UI-Module.

## Architektur-Anhang (Current)
...
=== Intake-Architektur (ergänzt durch R1964 am 2025-12-04 21:25:29) ===

Ziel
----
Dieser Abschnitt dokumentiert die reale Intake-Architektur von ShrimpDev
(Stand nach R1940/R195x/R1961/R1962) und ergänzt die bestehende
Architecture_ShrimpDev-Übersicht.

Aktive Intake-Kernmodule
------------------------

- logic_actions.py
  * Enthält zentrale Aktionen für den Intake (z.B. Speichern, Neu, Erkennen).
  * R1837-Block: action_save(app) -> speichert den Inhalt des Intake-Editors.
- logic_intake.py
  * Zusätzliche Logik rund um Intake/Editor und Hilfsfunktionen.
- logic_tools.py
  * Allgemeine Unterstützung für Runner / Tools.

Aktive Intake-UI-Module
-----------------------

- ui_left_panel.py
  * Baut das linke Intake-Panel mit:
    - Dateiname / Endung
    - Zielordner / Pfad
    - Buttons (Neu, Einfügen, Erkennen, Speichern, Undo, AOT, Restart, Log).
- ui_project_tree.py
  * Baut die rechte Projektliste:
    - Spalten: Name, Endung, Datum, Zeit.
    - Die Endung wird aus dem Dateinamen abgeleitet ('.py', '.cmd', ...).
- ui_leds.py
  * Steuert die LED-Anzeige (u.a. Syntax-Status) im Intake-Tab.
- ui_toolbar.py / ui_toolbar_dev.py
  * Toolbar / Buttons, die an logic_actions angebunden sind.

Historische / Legacy-Intake-Module
----------------------------------

Diese Module sind laut aktueller Architektur-Übersicht vorhanden,
aber dienen als historische oder alternative Intake-Varianten:

- module_code_intake.py
- module_code_intake_v1.py
- module_shim_intake.py

Sie werden vom aktuellen Intake-Aufbau nicht direkt verwendet und
sollen nur noch als Referenz oder für gezielte Migrationsschritte
verwendet werden.

Regeln für zukünftige Intake-Patches
------------------------------------

1. Funktionale Änderungen am Intake (Speichern, Erkennen, Syntax-Check)
   immer über:
   - logic_actions.py (z.B. R1837-Block: action_save, action_detect)
   - logic_intake.py
   - ui_left_panel.py und ui_project_tree.py für UI-Anpassungen.

2. LED- und Status-Anzeige über ui_leds.py ansteuern, nicht über
   direkte Prints in der UI.

3. module_code_intake.py, module_code_intake_v1.py und module_shim_intake.py
   als Legacy markieren und NICHT mehr als aktive Intake-Basis verwenden.

4. Architektur-Änderungen an Intake IMMER parallel in dieser Datei
   dokumentieren (idealerweise automatisiert durch einen Runner wie
   R1964), damit Snapshots und Realität synchron bleiben.

[R1981] 2025-12-07 19:50:52 – Intake-Erkennung verbessert:
- _on_paste: Nach dem Einfuegen wird automatisch _on_detect() aufgerufen.
- _on_detect: Heuristik fuer .py / .cmd / .txt basierend auf Inhalt und Kopfzeile.
- Verhalten: 'Erkennen' und 'Einfuegen' helfen bei der korrekten Zuordnung von Name/Endung.
...
- Sie dient als Snapshot der aktuellen Modul- und Runner-Struktur.
...
Ende der Architektur-Übersicht.
...
### Debug-Logging darf keine neuen Variablen einführen  <!-- MR-DEBUG-LOGGING-NO-NEW-VARS -->

- **Regel:** Debug-/Trace-Logging darf **keine** Variablen referenzieren, die in der aktuellen Funktion **nicht existieren** (kein "mal schnell" `name`, `busy`, etc.).
