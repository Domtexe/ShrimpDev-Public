# R2514 – Compile Diag ui_runner_products_tab.py (READ-ONLY)

- Time: 20251222_163629
- Target: `C:\Users\rasta\OneDrive\ShrimpDev\modules\ui_runner_products_tab.py`

## Compile
- py_compile: FAIL
- Error: `Sorry: IndentationError: unexpected indent (ui_runner_products_tab.py, line 869)`

## Markers
- L719: # R2497_ACTION_START
- L739: # --- R2511_START: Backup restore (SAFE) ----------------------------------------------
- L842: # --- R2511_END: Backup restore (SAFE) -----------------------------------------------
- L845: def _tree_context_menu(ev):
- L884: # --- R2508_START: Preview right-click menu ------------------------------------------

## Context around line 869
```text
   L00779:         p = _selected_path()
   L00780:         if not p:
   L00781:             return
   L00782:         if not _is_backup_path(p):
   L00783:             try:
   L00784:                 from tkinter import messagebox
   L00785:                 messagebox.showwarning("Backup wiederherstellen", "Auswahl ist kein Backup aus _Archiv (*.bak).")
   L00786:             except Exception:
   L00787:                 pass
   L00788:             return
   L00789: 
   L00790:         base = _infer_restore_basename(p)
   L00791:         cands = _find_restore_candidates(base)
   L00792: 
   L00793:         if len(cands) != 1:
   L00794:             # ambiguous or missing -> do not restore blindly
   L00795:             msg = f"Restore nicht möglich (Treffer={len(cands)}).\nBackup:\n{p}\n\nZiel-Dateiname:\n{base}\n"
   L00796:             if cands:
   L00797:                 msg += "\nKandidaten:\n- " + "\n- ".join(cands[:20])
   L00798:                 if len(cands) > 20:
...
>> L00869:                     m.add_separator()
   ...
   L00940:         except Exception:
   L00941:             pass
   L00942: 
   L00943:     try:
   L00944:         txt.bind("<Button-3>", _preview_context_menu)
   L00945:     except Exception:
   L00946:         pass
   L00947:     # --- R2508_END: Preview right-click menu --------------------------------------------
   L00948:         tree.bind("<Control-c>", _tree_copy_path)
   L00949:         tree.bind("<Control-Shift-C>", _tree_copy_content)
   L00950:     except Exception:
   L00951:         pass
   L00952:     # --- /R2303 TREE UX --------------------------------------------------------------
   L00953: 
   L00954:     def _refilter(*_a):
   L00955:         try:
   L00956:             filtered = _apply_filters(state.get("items") or [])
   L00957:             _fill_tree(filtered)
   L00958:         except Exception:
   L00959:             pass
```
