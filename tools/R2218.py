...
def scan_tools_root_pairs(tools_dir: Path) -> dict[int, RunnerPair]:
    ...
    for p in tools_dir.iterdir():
        ...
        if rid not in pairs:
            pairs[rid] = RunnerPair(
                rid=rid, py=None, cmd=None, mtime=mt, py_mtime=None, cmd_mtime=None
            )
...
def main() -> int:
    ...
    newest_ids = [
        rp.rid
        for rp in sorted(pairs.values(), key=lambda r: (r.mtime, r.rid), reverse=True)[:KEEP_LAST_N]
    ]
    ...
    md.append(
        f"- Sicherheits-Puffer: **die letzten {KEEP_LAST_N} Runner-Paare (mtime)** bleiben immer stehen.\n"
    )
    ...
    print(
        f"[{RID}] KEEP (buffer+keepfile): {len(keep)} | ARCHIVE: {len(archive)} | Total pairs: {len(pairs)}"
    )
