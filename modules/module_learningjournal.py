...
def _find_journal_file() -> tuple[Path | None, str]:
    """
...
def _load_learning_journal() -> tuple[list[dict[str, Any]], str]:
    ...
    try:
        ...
        msg = f"Fehler beim Lesen von {path}: {exc}"
        _log(msg)
        return [], msg

    entries = _normalize_entries(raw)
    _log(f"LearningJournal geladen: {len(entries)} Eintraege aus {path}")
    return entries, info


def learning_log_event(event_type: str, message: str, data: dict[str, Any] | None = None) -> None:
...
def _format_entry_json(entry: dict[str, Any]) -> str:
    ...
    try:
        ...
        return f"Fehler beim Formatieren des Eintrags:\n{exc}\n\nRohdaten:\n{repr(entry)}"
