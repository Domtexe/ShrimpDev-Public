...
class GraphAnalyzer:
    ...
    def compute_links(self)->List[Dict[str,Any]]:
        ...
        for e in hist:
            p=e.get("payload",{}) or {}
            mods=[str(m[0]) for m in p.get("module_heatmap",[]) if isinstance(m,(list,tuple)) and m]
            runs=[str(r[0]) for r in p.get("runner_frequency",[]) if isinstance(r,(list,tuple)) and r]
