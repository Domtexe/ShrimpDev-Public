...
def _run_cmd(app, filename: str, title: str):
    ...
    if not os.path.exists(path):
        try: app.set_status(f"{title}: {filename} nicht gefunden.")
        except Exception: pass
        return
    try:
        subprocess.Popen([path], cwd=os.path.dirname(path), shell=True)
        try: app.set_status(f"{title} gestartet.")
        except Exception: pass
    except Exception as e:
        try: app.set_status(f"{title}: Fehler: {e}")
        except Exception: pass

def tool_futurefix(app):       _run_cmd(app, "R9997.cmd", "FutureFix")
def tool_futurefix_safe(app):  _run_cmd(app, "R1351.cmd", "FutureFix Safe")
def tool_learningjournal(app): _run_cmd(app, "R1252.cmd", "LearningJournal")
def tool_activate(app):        _run_cmd(app, "Activate.cmd", "Activate")
def tool_r9998(app):           _run_cmd(app, "R9998.cmd", "R9998")
def tool_r9999(app):           _run_cmd(app, "R9999.cmd", "R9999")
