...
def action_run(app) -> None:
    """Startet die aktuell gespeicherte Intake-Datei IMMER über module_runner_exec (zentraler Log-Pfad)."""
    import os as _os
    ...
    # Endung normalisieren: Intake darf ohne Punkt arbeiten
    ...
    try:
        ...
        try:
            _learning_log_event(app, "intake_run", "ok_central")  # type: ignore[name-defined]
        except Exception:
            pass
    except Exception as exc:
        _rR1837_status(app, f"Run fehlgeschlagen: {exc}")
        try:
            _learning_log_event(app, "intake_run", "error")  # type: ignore[name-defined]
        except Exception:
            pass
        return
...
#     if not root:
#         return None
#     reports = _r1849_os.path.join(root, "Reports")
#     try:
