...
def create_buttons(parent, app):
    frm = tk.Frame(parent); frm.pack(anchor="w", padx=12, pady=10)
    ...
    tk.Button(frm, text="Reload Modules",command=lambda: _reload(app)).grid(row=0, column=2, padx=6)
    tk.Button(frm, text="Run Runner...", command=lambda: _run_runner(app)).grid(row=0, column=3, padx=6)
...
def _reload(app):
    try:
        ...
        for mod in ("modules.ui_menus","modules.ui_buttons","modules.ui_lists","modules.ui_statusbar","modules.ui_themes","modules.logic_intake"):
