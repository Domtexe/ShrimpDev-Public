...
def build_menu(app):
    ...
    file.add_checkbutton(label="Always on Top",
                         command=lambda: _toggle_top(app))
    file.add_separator()
    file.add_command(label="Beenden", command=app.destroy)
    m.add_cascade(label="File", menu=file)

    helpm = tk.Menu(m, tearoff=False)
    helpm.add_command(label="Info", command=lambda: messagebox.showinfo("ShrimpDev", "ShrimpDev - Development GUI"))
