...
def _build_tab(parent):
    ...
    def run_sel(_evt=None):
        ...
        try:
            subprocess.Popen(cmd, cwd=str(ROOT), shell=False)
        except Exception as ex:
            try:
                messagebox.showerror("ShrimpDev", f"Startfehler:\n{ex}")
            except Exception:
                pass
...
def open_runner_board(app: tk.Tk) -> bool:
    try:
        return ensure_tab(app, "runners", "Runner Board", _build_tab)
    except Exception as ex:
        try:
            messagebox.showerror("ShrimpDev", f"Runner Board Fehler:\n{ex}")
        except Exception:
            pass
        return False
