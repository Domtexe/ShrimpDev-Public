...
OUT = Path(r"D:\ShrimpDev\Reports\ProjectMap")
OUT.mkdir(parents=True, exist_ok=True)
MAP_JSON = OUT / "project_map.json"
MAP_HTML = OUT / "project_map.html"

IGNORE_DIRS = {"_Archiv", "Reports", ".git", "dist", "build", "__pycache__"}
...
def _py_info(p: Path) -> dict[str, Any]:
    ...
    for m in re.finditer(
        r'\.bind[_a-z]*\(\s*[^,]+,\s*[\'"]([^\'"]+)[\'"]\s*,\s*([A-Za-z_]\w*)', src
    ):
...
def scan() -> dict[str, Any]:
    ...
    for p in HUB.rglob("*"):
        ...
        if p.suffix.lower() == ".py":
            files.append(_py_info(p))
        else:
            files.append(_bat_info(p))
